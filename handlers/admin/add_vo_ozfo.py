...
async def update_vo(msg: types.Message):
    ...
    await vo_ozfo.vo_ozfo_step_start.set()
@dp.message_handler(state=vo_ozfo.vo_ozfo_step_start)
async def add_vo(msg: types.Message, state: FSMContext):
    if msg.text != "Отмена":
        await command.add_vo_ozfo(msg.text)
    await state.finish()


@dp.message_handler(IsAdmin(), text='/update_vo_ozfo')
async def update_spo(msg: types.Message):
    ...
    await vo_ozfo.vo_ozfo_step_1.set()

@dp.message_handler(state=vo_ozfo.vo_ozfo_step_1)
async def state1(msg: types.Message, state: FSMContext):
    answer1 = msg.text
    data.append(answer1)
    await msg.answer("Напишите есть ли бюджетные места")
    await vo_ozfo.vo_ozfo_step_2.set()


@dp.message_handler(state=vo_ozfo.vo_ozfo_step_2)
async def state1(msg: types.Message, state: FSMContext):
    answer2 = msg.text
    data.append(answer2)
    ...
    await vo_ozfo.vo_ozfo_step_3.set()

@dp.message_handler(state=vo_ozfo.vo_ozfo_step_3)
async def state2(msg: types.Message, state: FSMContext):
    answer3 = msg.text
    data.append(answer3)

    await msg.answer(f"Укажите цену за год в рублях")
    await vo_ozfo.vo_ozfo_step_4.set()

@dp.message_handler(state=vo_ozfo.vo_ozfo_step_4)
async def state2(msg: types.Message, state: FSMContext):
    answer4 = msg.text
    data.append(answer4)

    await msg.answer(f"Какие экзамены необходимы для поступления после 11 класса?")
    await vo_ozfo.vo_ozfo_step_5.set()

@dp.message_handler(state=vo_ozfo.vo_ozfo_step_5)
async def state2(msg: types.Message, state: FSMContext):
    answer5 = msg.text
    data.append(answer5)

    await msg.answer(f"Какие вступительные экзамены необходимы для поступления?")
    await vo_ozfo.vo_ozfo_step_6.set()

@dp.message_handler(state=vo_ozfo.vo_ozfo_step_6)
async def state2(msg: types.Message, state: FSMContext):
    answer6 = msg.text
    data.append(answer6)
    await msg.answer(f"Хотите оставить какой-нибудь комментарий, если нет, то просто напишите \"<b>нет</b>\"")
    await vo_ozfo.vo_ozfo_step_7.set()


@dp.message_handler(state=vo_ozfo.vo_ozfo_step_7)
async def state2(msg: types.Message, state: FSMContext):
    answer7 = msg.text
    ...
    await vo_ozfo.vo_ozfo_step_finish.set()

@dp.message_handler(state=vo_ozfo.vo_ozfo_step_finish)
async def state2(msg: types.Message, state: FSMContext):
    if msg.text == "Готово":
        await command.update_edu_prog_vo_ozfo(data[0], data[1], data[2], data[3], data[4], data[5], data[6])
