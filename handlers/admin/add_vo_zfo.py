...
from states import vo_vo_zfo, vo11_zfo, vo_spo_zfo
...
@dp.message_handler(state=vo_spo_zfo.vo_spo_zfo_step_7)
async def update_spo(msg: types.Message):
    kb_spo = ReplyKeyboardMarkup(
        keyboard=[
            [
...
async def add_vo(msg: types.Message, state: FSMContext):
    ...
    if msg.text == "СПО":
        await vo_spo_zfo.vo_spo_zfo_step_1.set()
    if msg.text == "ВО":
        await vo_vo_zfo.vo_vo_zfo_step_1.set()
...
async def state2(msg: types.Message, state: FSMContext):
    if msg.text == "Готово":
        await command.update_edu_prog_vo11_zfo(data[0], data[1], data[2], data[3], data[4])
        data.clear()
...
@dp.message_handler(state=vo_spo_zfo.vo_spo_zfo_step_1)
async def state1(msg: types.Message, state: FSMContext):
    answer1 = msg.text
    data.append(answer1)
    await msg.answer("Напишите есть ли бюджетные места")
    await vo_spo_zfo.vo_spo_zfo_step_3.set()

@dp.message_handler(state=vo_spo_zfo.vo_spo_zfo_step_3)
async def state1(msg: types.Message, state: FSMContext):
    answer3 = msg.text
    data.append(answer3)
    ...
    await vo_spo_zfo.vo_spo_zfo_step_4.set()

@dp.message_handler(state=vo_spo_zfo.vo_spo_zfo_step_4)
async def state2(msg: types.Message, state: FSMContext):
    answer4 = msg.text
    data.append(answer4)

    await msg.answer(f"Укажите цену за год в рублях")
    await vo_spo_zfo.vo_spo_zfo_step_5.set()

@dp.message_handler(state=vo_spo_zfo.vo_spo_zfo_step_5)
async def state2(msg: types.Message, state: FSMContext):
    answer5 = msg.text
    data.append(answer5)

    await msg.answer(f"Какие экзамены или вступительные экзамены необходимы для поступления?")
    await vo_spo_zfo.vo_spo_zfo_step_6.set()


@dp.message_handler(state=vo_spo_zfo.vo_spo_zfo_step_6)
async def state2(msg: types.Message, state: FSMContext):
    answer6 = msg.text
    data.append(answer6)
    await msg.answer(f"Пожалуйста проверьте, всё ли заполнено правильно:\n"
    ...
    await vo_spo_zfo.vo_spo_zfo_step_finish.set()


@dp.message_handler(state=vo_spo_zfo.vo_spo_zfo_step_finish)
async def state2(msg: types.Message, state: FSMContext):
    if msg.text == "Готово":
        await command.update_edu_prog_vo_on_spo_zfo(data[0], data[1], data[2], data[3], data[4])
        data.clear()
        await msg.answer(f"Изменения сохранены", reply_markup=types.ReplyKeyboardRemove())
        await vo_spo_zfo.vo_spo_zfo_step_finish.set()
    else:
...
@dp.message_handler(state=vo_vo_zfo.vo_vo_zfo_step_1)
async def state1(msg: types.Message, state: FSMContext):
    answer1 = msg.text
    data.append(answer1)
    await msg.answer("Напишите есть ли бюджетные места")
    await vo_vo_zfo.vo_vo_zfo_step_3.set()

@dp.message_handler(state=vo_vo_zfo.vo_vo_zfo_step_3)
async def state1(msg: types.Message, state: FSMContext):
    answer3 = msg.text
    data.append(answer3)
    ...
    await vo_vo_zfo.vo_vo_zfo_step_4.set()

@dp.message_handler(state=vo_vo_zfo.vo_vo_zfo_step_4)
async def state2(msg: types.Message, state: FSMContext):
    answer4 = msg.text
    data.append(answer4)

    await msg.answer(f"Укажите цену за год в рублях")
    await vo_vo_zfo.vo_vo_zfo_step_5.set()
...
@dp.message_handler(state=vo_vo_zfo.vo_vo_zfo_step_6)
async def state2(msg: types.Message, state: FSMContext):
    answer6 = msg.text
    data.append(answer6)
    await msg.answer(f"Пожалуйста проверьте, всё ли заполнено правильно:\n"
                     ...
                     f"Если всё готово, нажмите на кнопку \"Готово\", если Вы допустили ошибку, нажмите \"Отмена\"",
                     reply_markup=kb_confirm)
    await vo_vo_zfo.vo_vo_zfo_step_finish.set()


@dp.message_handler(state=vo_vo_zfo.vo_vo_zfo_step_finish)
async def state2(msg: types.Message, state: FSMContext):
    if msg.text == "Готово":
        await command.update_edu_prog_vo_on_vo_zfo(data[0], data[1], data[2], data[3], data[4])
        data.clear()
        await msg.answer(f"Изменения сохранены", reply_markup=types.ReplyKeyboardRemove())
        await vo_vo_zfo.vo_vo_zfo_step_finish.set()
    else:
