...
async def update_vo(msg: types.Message):
    ...
    await msg.answer("Введите название образовательной программы, которую Вы хотите добавить, либо нажмите отмена", reply_markup=kb_vo)
...
async def update_spo(msg: types.Message):
    ...
    await msg.answer("Вы хотите исправить список ВО после 9 или 11?", reply_markup=kb_vo)
...
async def state1(msg: types.Message, state: FSMContext):
    ...
    data.append(answer1)
    await msg.answer("Напишите есть ли бюджетные места")
    await vo_spo_zfo.vo_spo_zfo_step_2.set()

@dp.message_handler(state=vo_spo_zfo.vo_spo_zfo_step_2)
async def state1(msg: types.Message, state: FSMContext):
    answer2 = msg.text
    data.append(answer2)
    await msg.answer("Какие присутсвуют формы обучения?")
